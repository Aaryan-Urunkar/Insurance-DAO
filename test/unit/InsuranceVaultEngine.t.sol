/**
 * @title InsuranceVaultEngineTest
 * @author Aaryan Urunkar
 * @notice Unit tests to see if basic operations of Insurance DAO engine are working or not
 *
 * @notice @dev NOTICE: Before running this PLEASE make sure you do a forked test for SEPOLIA
 *                      And change the visibility of _withdrawClaim(address) in InsuranceVaultEngine.sol to external just for the purposes of this test
 */
...
contract Name is Test {
    ...
    function testIfWithdrawCase1Works() external multipleDepositsValidForClaims {
        ...
        uint256 totalClaim = engine._withdrawClaim(USER);
        vm.stopPrank();
    ...
    function testIfWithdrawCase2Works() external multipleDepositsValidForClaims {
        ...
        vault.approve(address(engine), type(uint256).max);
        uint256 totalClaim = engine._withdrawClaim(newUser1);
        vm.stopPrank();
    ...
    function testLiquidateRevertsIfLiquidationFeeIsLow() external multipleDepositsValidForClaims {
        vm.startPrank(USER);
        vm.expectRevert(
            InsuranceVaultEngine.InsuranceVaultEngine__PaidLessThanMinimumLiquidationFeeToLiquidate.selector
        );
